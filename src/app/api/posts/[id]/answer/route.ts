...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const systemPrompt = isZh
    ? `你是一位知識淵博的新聞助理。根據文章內容，用${isTW ? "繁體中文" : "简体中文"}以一段話回答讀者的問題。回答應清晰、準確，適合高中生閱讀程度。`
    : "You are a knowledgeable news assistant. Answer the reader's question based on the article in one clear paragraph. Write for a high school reading level.";
