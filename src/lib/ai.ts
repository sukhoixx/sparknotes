...
const CATEGORY_SELECTION_PROMPTS: Record<Category, string> = {
  ...
  taiwan: `You are the senior editor for Taiwan news coverage. Select articles covering significant domestic Taiwan news: politics, government policy, society, economy, cross-strait relations, elections, and major local events. Articles may be in Chinese — select based on newsworthiness regardless of language. Reject pure entertainment, celebrity gossip, sports scores, and trivial local incidents. Return ONLY a JSON array of selected indices (1-based). No explanation.`,
};
