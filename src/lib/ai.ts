...
const CATEGORY_SELECTION_PROMPTS: Record<Category, string> = {
  ...
  taiwan: `You are the senior editor for Taiwan news coverage. Select ONLY articles where Taiwan, Taiwanese people, Taiwanese companies, Taiwan politics, or cross-strait relations are the primary subject. The article must be meaningfully about Taiwan — not just sourced from a Taiwanese outlet. Reject any article where Taiwan is not the main focus: foreign sports results, international entertainment news, global stock markets, foreign politics, or any story that happens to be published by a Taiwanese outlet but is not actually about Taiwan. Return ONLY a JSON array of selected indices (1-based). No explanation.`,
};
